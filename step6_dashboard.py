...
def get_all_teams_avg_height_weight() -> pd.DataFrame:
    ...
    out = out[["team", "avg_height", "avg_weight",
               "players_count"]].sort_values("team")
    return out
...
def update_dashboard(team_value, season_value):
    ...
    try:
        ...
        if "error" in team_avgs.columns:
            ...
            team_avgs_cols = [{"name": c, "id": c} for c in team_avgs.columns]
        ...
        if "error" in leader.columns:
            ...
            leader_cols = [{"name": c, "id": c} for c in leader.columns]
        ...
        if "error" in recent.columns:
            ...
            recent_cols = [{"name": c, "id": c} for c in recent.columns]
